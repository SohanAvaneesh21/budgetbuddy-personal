        ...
        savingData: {
          $let: {
            vars: {
              ...
              investments: { $ifNull: ["$totalInvestments", 0] },
            },
            in: {
              // ((income - expenses + investments) / income) * 100; (investments are considered as savings)
              savingsPercentage: {
                $cond: [
                  { $lte: ["$$income", 0] },
                  0,
                  {
                    $multiply: [
                      {
                        $divide: [
                          { $add: [{ $subtract: ["$$income", "$$expenses"] }, "$$investments"] },
                          "$$income",
                        ],
                      },
                      100,
              ...
              //Expense Ratio = (expenses / income) * 100 (excluding investments)
              expenseRatio: {
                $cond: [
                  { $lte: ["$$income", 0] },
                  0,
